...
export async function captureAndPersist(opts) {
  ...
  // Don't persist empty snapshots — these usually happen when our startup
  // handler races Chrome's session-restore (Chrome hasn't opened any windows
  // yet). The live snapshot heartbeat + tab events will fill the history in.
  // Manual saves explicitly bypass this guard.
  if (snap.stats.tabCount === 0 && (opts?.type === 'auto' || opts?.type === 'startup' || opts?.type === 'live')) {
    return snap;
  }
