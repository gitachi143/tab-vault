...
import { getAllSnapshots, getSettings, getProfileId, getBrowserInfo, shortProfileId } from './storage.js';
...
export async function buildBackupPayload({ reason = 'hourly' } = {}) {
  ...
  return {
    ...
    browserName: browserInfo.name || 'Chrome',
...
export async function postToWebhook(url, payload, { secret = '', timeoutMs = 30_000 } = {}) {
  ...
  const body = JSON.stringify({
    ...
    browserName: payload.browserName || 'Chrome',
